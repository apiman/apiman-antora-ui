; (function () {
  ...
  function activateTab (e, skipAncestors) {
    ...
    const tabs = find(tabsetHash + ' > .content > .tabs li, ' +
      tabsetHash + ' > .content > .tabset-panes > .content > .tab-pane', this.tabset)
    ...
    if (!skipAncestors) {
      ...
      if (lastHash !== tabHash) {
        // prevents scrolling by remembering the current position
        const scrollTo = window.scrollY

        // update the location
        const currentUrl = window.location
        currentUrl.hash = tabHash
        window.history.pushState({}, '', currentUrl)

        // when changing the location ... the most browsers will scroll to the new location
        // force scrolling now. this will stop annoying flicker
